    ...
    def check_bbox(self):
        ...
        if any(v is not None for v in vals):
            if any(v is None for v in vals):
                raise HTTPException(status_code=400, detail="分析范围需同时提供 min_lon、min_lat、max_lon、max_lat")
            if self.min_lon >= self.max_lon or self.min_lat >= self.max_lat:
                raise HTTPException(status_code=400, detail="分析范围 min 值必须小于 max 值")
        return self
...
async def _fetch_poi_coords(db: AsyncSession, userid: int, name_keyword: str | None = None, bbox: tuple | None = None):
    """取点位坐标，支持名称关键字模糊过滤与分析范围过滤，返回 id/name/lon/lat 行列表"""
    stmt = select(
...
async def _kde_analysis(rows, grid_size: int, bandwidth: float | None = None, bounds: tuple | None = None):
    """核密度分析：高斯核 KDE，密度(c)=Σ exp(-d²/2σ²)，分块累加后归一化 0~1，返回热力格网 GeoJSON"""
    ...
    return {"type": "FeatureCollection", "features": features}, bounds, bandwidth
...
async def _kmeans_analysis(db: AsyncSession, userid: int, k: int, name_keyword: str | None = None, bbox: tuple | None = None):
    """POI 聚类：优先 PostGIS ST_ClusterKMeans（3857 投影），不支持时降级 numpy K-Means"""
    ...
    try:
        # ST_ClusterKMeans 窗口函数（PostGIS 3.0+），聚类编号从 0 开始
        stmt = select(
...
async def kde_analysis(
    ...
    rows = await _fetch_poi_coords(db=db, userid=user.userid, name_keyword=query.name_keyword, bbox=bbox)
    if len(rows) < 2:
        raise HTTPException(status_code=400, detail=f"符合条件的点位不足（{len(rows)} 个），核密度分析至少需要 2 个点")

    grid, bounds, bandwidth = await _kde_analysis(
